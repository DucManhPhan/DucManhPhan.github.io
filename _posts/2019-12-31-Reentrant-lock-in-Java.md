...
| When we go outside of of ```synchronized``` block or method, it will not re-enter to another ```synchronized``` block that using the same object. | With ```ReentrantLock```, we can acquire lock in method A and release in method B on the same lock object. |
| ```synchronized``` constructs  | A ```ReentrantLock``` unstructured, i.e. we do not need to use a block structure for locking and can even hold a block across methods. |
