...
- $\mathbf{\bar{X}}$ is a matrix that contains the training examples that are row vectors.
    ...
    Firstly, we can use a transpose of column vector that is $\mathbf{x} = \begin{bmatrix}x_0 \\ x_1 \\ \vdots \\ x_n \end{bmatrix}$ or $\mathbf{x} = \begin{bmatrix}x_0; \ x_1; \ \dots; \ x_n \end{bmatrix}$ with $x_0 = 1$
